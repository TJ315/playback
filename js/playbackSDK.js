...
!(function ($, window, document) {
    ...
    // 播放器加载完成，开始播放
    window.on_cc_live_player_init = function (opts) {
        callback.callbackPlayer.isReady = true;
        try {
        ...
        if (typeof window.inset_on_cc_live_player_load === "function") {
            window.inset_on_cc_live_player_load(opts)
        }
    ...
    window.seekComplete = function () {   	    	     	 
        ...
        window.inset_seekComplete && window.inset_seekComplete();//插播视频收到 打点成功方法
    };
        ...
        appendVideo: function (s, opts) {
            ...
            Event.addEvents(video, "canplay", function () {
                if (MobileLive.isMobile() == "isMobile") {
                    ...
                    window.inset_on_cc_live_player_load && window.inset_on_cc_live_player_load(opts)
                    window.on_cc_h5_player_load && window.on_cc_h5_player_load();
                } else if (DW.isH5play) {
                    window.on_cc_live_player_init && window.on_cc_live_player_init(opts);
                }
            ...
            Event.addEvents(video, "playing", function () {
                ...
                window.inset_on_player_start &&inset_on_player_start();
                
                window.on_spark_player_resume && on_spark_player_resume();
                window.inset_on_spark_player_resume && inset_on_spark_player_resume();
            ...
            Event.addEvents(video, "pause", function () {
                ...
                window.inset_on_spark_player_pause && inset_on_spark_player_pause();
            }, false);

            Event.addEvents(video, "ended", function () {
                window.on_spark_player_end && on_spark_player_end();
                window.inset_on_spark_player_end && inset_on_spark_player_end();
            }, false);
